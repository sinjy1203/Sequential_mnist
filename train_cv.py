...
def generate_model(model):
    if model == 'TCN':
        ...
        Model = TCN(input_size=1, hidden_size=hidden_size, output_size=10,
                    ...
                    kernel_size=kernel_size).double().to(device, dtype=torch.float32)
    elif model == 'LSTM' or model == 'GRU':
        rnn = True
        Model = RNN(input_size=28, hidden_size=hidden_size, output_size=10,
                     num_layers=num_layers, dropout=dropout, bi=bi, cell=model).double().to(device, dtype=torch.float32)
    else:
...
for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
    ...
    for epoch in range(num_epochs):
        ...
        writer.add_scalars("Loss", {str(fold): epoch_test_loss}, epoch)
        writer.add_scalars("Acc", {str(fold): epoch_test_acc}, epoch)

    final_acc += early_stopping.val_acc_best / k_folds
    final_loss += early_stopping.val_loss_min / k_folds

print("LOSS: {} || ACC: {}".format(final_loss, final_acc))
